...
if __name__ == '__main__':
    #region 参数解析
    parser = argparse.ArgumentParser(description='Telegram 频道信息获取')
    parser.add_argument('--channel', required=True, help="频道名称")
    parser.add_argument('--alias', required=False, default="None", help="频道别名")
    parser.add_argument('--no-source', action='store_true', help='是否无法辨别消息真实性')
    args = parser.parse_args()
    #endregion

    rss_url = f'http://192.168.238.128:1200/telegram/channel/{args.channel}/'
    ...
    for entry in feed.entries:
        ...
        # other meta
        data['table_type'] = 'rss'
        data['platform'] = args.alias if args.alias != "None" else args.channel
        data['rss_type'] = 'Telegram频道'
        data['meta'] = ['无法辨别消息真实性'] if args.no_source else ['官方频道/群组']
